    ...
    var c = {
        ...
        tabAdd: function (i) {
            ...
            if (t.tabExists(s))
            {
                ...
                var bb = t._title.children("li[class=layui-this]").attr("lay-id");
             

                var ss = t._config.elem;
                var cc = $(ss).find('.kit-tab div.layui-tab-content');
                var dd = cc.children("div[lay-item-id=" + bb + "]").children("iframe");
                dd.attr("src", dd.attr("src"));
              //  var aa = t._content.children("div[lay-item-id=" + t._config + "]").children("iframe");
               // aa.attr("src", aa.attr("src"));
            }
               
            else {
                NProgress.start();
                //修改：去掉星号图标 by Sunlz on 2018/05/02
                //var r = ['<li class="layui-this" lay-id="' + s + '" >']; -1 !== n.indexOf("fa-") ? r.push('<i class="fa ' + n + '" aria-hidden="true"></i>') : r.push('<i class="layui-icon">' + n + "</i>"),
                var r = ['<li class="layui-this" lay-id="' + s + '" >']; r.push(""),
                r.push("&nbsp;" + l),
